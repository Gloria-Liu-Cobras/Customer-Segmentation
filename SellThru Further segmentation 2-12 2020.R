...
C<-"SELECT NON EMPTY { [Measures].[Sales Amount], [Measures].[Qty Sold] } ON COLUMNS, NON EMPTY { ([Customer].[Group Name].[Group Name].ALLMEMBERS * [High Volume Trans].[High Volume].[High Volume].ALLMEMBERS * [Item].[Short SKU].[Short SKU].ALLMEMBERS * [Order Details].[Invoice Number].[Invoice Number].ALLMEMBERS * [Date].[Date].[Date].ALLMEMBERS * [Item].[How Is it Bought].[How Is it Bought].ALLMEMBERS * [Customer].[Customer Country].[Customer Country].ALLMEMBERS * [Customer].[Customer Region].[Customer Region].ALLMEMBERS ) } DIMENSION PROPERTIES MEMBER_CAPTION, MEMBER_UNIQUE_NAME ON ROWS 
FROM ( SELECT ( Filter( [Date].[Fiscal Calendar].ALLMEMBERS - [Date].[Fiscal Calendar].[(All)].ALLMEMBERS, Instr( [Date].[Fiscal Calendar].currentmember.Properties( 'Member_Caption' ), '2018' )  = 1  ) ) ON COLUMNS
...
C<-"SELECT NON EMPTY { [Measures].[Sales Amount], [Measures].[Qty Sold] } ON COLUMNS, NON EMPTY { ([Customer].[Group Name].[Group Name].ALLMEMBERS * [High Volume Trans].[High Volume].[High Volume].ALLMEMBERS * [Item].[Short SKU].[Short SKU].ALLMEMBERS * [Order Details].[Invoice Number].[Invoice Number].ALLMEMBERS * [Date].[Date].[Date].ALLMEMBERS * [Item].[How Is it Bought].[How Is it Bought].ALLMEMBERS * [Customer].[Customer Country].[Customer Country].ALLMEMBERS * [Customer].[Customer Region].[Customer Region].ALLMEMBERS ) } DIMENSION PROPERTIES MEMBER_CAPTION, MEMBER_UNIQUE_NAME ON ROWS 
FROM ( SELECT ( Filter( [Date].[Fiscal Calendar].ALLMEMBERS - [Date].[Fiscal Calendar].[(All)].ALLMEMBERS, Instr( [Date].[Fiscal Calendar].currentmember.Properties( 'Member_Caption' ), '2019' )  = 1  ) ) ON COLUMNS
FROM ( SELECT ( { [Transaction Type].[Transaction Type].&[1], [Transaction Type].[Transaction Type].&[3] } ) ON COLUMNS 
FROM [SellThru])) WHERE ( [Transaction Type].[Transaction Type].CurrentMember )
"
Customer2 <- execute2D(olapCnn, C)
Customer2 <- Customer2[c(1,3,5,7,9,11,13,15,17,18)]
colnames(Customer2)<-c("CustomerName","HighVolume","SKU","Invoice_Number","Order_Date","HowIsItBought","Country","Region","Sales_Amount","Qty_Sold")
Customer2$Order_Date<-as.Date(Customer2$Order_Date,format="%m/%d/%y")

rfmdata2018<-Customer%>%filter(year(Order_Date)==2018)%>%select(CustomerName,SKU,Order_Date,Sales_Amount,Qty_Sold,Invoice_Number)
rfmdata2019<-Customer2%>%filter(year(Order_Date)==2019)%>%select(CustomerName,SKU,Order_Date,Sales_Amount,Qty_Sold,Invoice_Number)

length(unique(rfmdata2018$CustomerName)) #56888
length(unique(rfmdata2019$CustomerName)) #61101


detach("package:plyr", unload=TRUE) 
df_rfm<-rfmdata2019%>%filter(Sales_Amount>0)%>%group_by(CustomerName)%>%
...
df_rfm18<-rfmdata2018%>%filter(Sales_Amount>0)%>%group_by(CustomerName)%>%
  summarize(Avg_UnitsPerTrxn=sum(Qty_Sold)/n_distinct(Invoice_Number),
            InvPerCust=n_distinct(Invoice_Number),
            Avg_DolPerTrxn=sum(Sales_Amount)/n_distinct(Invoice_Number),
            Dist_SKUPerCust=n_distinct(SKU),monetary=sum(Sales_Amount))
...
ic <-c(45.6,2079.744)
ad <-c(30.87,935.3671)
ds <-c(14.43997,208.5127)
#Get the score
df2<-getScoreWithBreaks(df_rfm,au,ic,ad,ds)
df18<-getScoreWithBreaks(df_rfm18,au,ic,ad,ds)


length(table(df2$Total_Score))
length(table(df18$Total_Score))
...
rfm_segments <- df20  %>% group_by(CustomerName)%>%mutate(
  segment = case_when(
    ...
    (AD_Score %>% between(2, 3)) & (IC_Score ==3) &
      (AU_Score %>% between(1,2)) & (DS_Score %>% between(2, 3))  ~ "Main E-resellers",
    
    (AD_Score  %>% between(2, 3)) & (IC_Score  %>% between(1, 2)) &
      (AU_Score  %>% between(2, 3)) & (DS_Score %>% between(2,3))  ~ "Front Office Resellers",
    
    
    
    (AD_Score  %>% between(2, 3)) & (IC_Score  ==1) &
      (AU_Score ==1) & (DS_Score==1) ~ "Back Office Resellers",
    (AD_Score == 2) & (IC_Score ==2) &
      (AU_Score %>% between(2,3 )) & (DS_Score %>% between(1,2)) ~ "Mixed Technology Loyal Customer",
    (AD_Score == 3) & (IC_Score ==2) &
      (AU_Score %>% between(2,3 )) & (DS_Score %>% between(1,2)) ~ "Technology Focused High Value Company",
    ...
    (AD_Score  %>% between(2, 3)) & (IC_Score  %>% between(1, 2)) &
      (AU_Score  %>% between(1, 2))  ~ "Mixed Resellers",
    (Total_Score==3131) ~ "High Value Triers",
    (Total_Score==1213 ) | (Total_Score==1113) ~ "Multi-SKUs Triers",
    TRUE ~ "Triers"
  )
) 
...
write.csv(as.data.frame(rfm_segments),"SellThru2019.csv")
